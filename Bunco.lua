...
local function forced_message(message, card, color, delay, juice)
	...
	event({
		...
		func = function()
			...
			card_eval_status_text(card, "extra", nil, nil, nil, { message = message, colour = color, instant = true })
			return true
		end,
	})
end
...
BUNCOMOD.funcs.get_exotic_suits = function()
	local suits = {
		"bunc_Fleurons",
		"bunc_Halberds",
	}

	-- Paperback compatibility
	if SMODS and SMODS.Suits then
		if SMODS.Suits["paperback_Stars"] then
			suits[#suits + 1] = "paperback_Stars"
		end

		if SMODS.Suits["paperback_Crowns"] then
...
BUNCOMOD.funcs.is_exotic_suit = function(card)
	if not card or SMODS.has_no_suit(card) then
		return false
	end

	for _, suit in ipairs(BUNCOMOD.funcs.get_exotic_suits()) do
...
BUNCOMOD.funcs.is_base_exotic_suit = function(card)
	if not card or not card.base or SMODS.has_no_suit(card) then
		return false
	end
...
function BUNCOMOD.content.config_tab()
	return {
		...
		nodes = {
			{
				n = G.UIT.C,
				config = { r = 0.1, minw = 4, align = "tl", padding = 0.2, colour = G.C.BLACK },
				nodes = {
					{
						n = G.UIT.R,
						config = {
						...
						nodes = {
							create_toggle({
								...
								info = {
									G.localization.misc.dictionary.bunc_default_true
										.. ", "
										.. G.localization.misc.dictionary.bunc_requires_restart,
									"",
							...
							nodes = {
								{
									...
									nodes = {
										create_slider({
											label = G.localization.descriptions.Joker.j_bunc_trigger_finger.name
												.. " ("
												.. G.localization.misc.dictionary.bunc_volume
												.. ")",
											w = 5,
											h = 0.4,
											ref_table = G.SETTINGS.SOUND,
											ref_value = "bunc_trigger_finger_volume",
											min = 0,
											max = 100,
										}),
									},
								},
								{
									...
									nodes = {
										create_slider({
											label = G.localization.descriptions.Joker.j_bunc_mousetrap.name
												.. " ("
												.. G.localization.misc.dictionary.bunc_volume
												.. ")",
											w = 5,
											h = 0.4,
											ref_table = G.SETTINGS.SOUND,
											ref_value = "bunc_mousetrap_volume",
											min = 0,
											max = 100,
										}),
									},
								},
								{
									...
									nodes = {
										create_slider({
											label = G.localization.descriptions.Joker.j_bunc_stylophone.name
												.. " ("
												.. G.localization.misc.dictionary.bunc_volume
												.. ")",
											w = 5,
											h = 0.4,
											ref_table = G.SETTINGS.SOUND,
											ref_value = "bunc_stylophone_volume",
											min = 0,
											max = 100,
										}),
									},
									...
									nodes = {
										{
											n = G.UIT.T,
											config = {
												text = G.localization.misc.dictionary.b_credits,
...
for index = 1, 5 do
	SMODS.ScreenShader({
		...
		send_vars = function(self)
			...
			if BUNCOMOD.content.config.high_quality_shaders and card and not card.removed and card.states.visible then
				...
				return {
					...
					screen_size = { w, h },
					radius = (card.config and card.config.forcefield and card.config.forcefield.radius or 160)
						* (1.2 + math.sin((index * 3.0) + G.TIMERS.REAL / 2.0) / 3.0)
						* dissolve_mult,
					strength = (card.config and card.config.forcefield and card.config.forcefield.strength or -0.5)
						* (0.8 - math.cos((index * 3.0) + G.TIMERS.REAL / 2.0) / 3.0)
						* dissolve_mult,
				}
...
SMODS.Consumable:take_ownership("pluto", {
	set_card_type_badge = function(self, card, badges)
		badges[1] = create_badge(
			config.fixed_badges and localize("k_planet") or localize("k_dwarf_planet"),
			get_type_colour(self or card.config, card),
			nil,
			1.2
		)
	end,
...
SMODS.Consumable:take_ownership("ceres", {
	set_card_type_badge = function(self, card, badges)
		badges[1] = create_badge(
			config.fixed_badges and localize("k_planet_q") or localize("k_dwarf_planet"),
			get_type_colour(self or card.config, card),
			nil,
			1.2
		)
			...
			if context.skipping_booster and not context.blueprint then
				...
				return {
					message = localize({
						type = "variable",
						key = "a_mult",
						vars = { card.ability.extra * G.GAME.pack_choices },
					}),
					colour = G.C.RED,
					delay = 0.45,
				}
	...
	SMODS.Tag:take_ownership("boss", {
		loc_vars = function(self, info_queue)
			info_queue[#info_queue + 1] =
				{ key = "p_bunc_blind", set = "Other", vars = { G.P_CENTERS.p_bunc_blind_1.config.extra } }
...
function Card:get_chip_bonus()
	return Card_get_chip_bonus(self)
		+ (
			self.ability
				and type(self.ability) == "table"
				and not self.debuff
				and self.ability.temporary_extra_chips
			or 0
		)
end
...
-- Repetitions unlocking / Fleuron retriggers
...
SMODS.calculate_repetitions = function(card, context, reps)
	...
	if context.cardarea == G.play
		...
		and card:is_suit("bunc_Fleurons")
		and SMODS.pseudorandom_probability(
			card,
			"bunc_fleuron_retrigger",
			1,
			3
		)
	then
	...
	check_for_unlock({
		type = "repetition",
		repetition_amount = #reps - 1
	})
end

-- Halberd suit effect
local bunc_original_eval_card = eval_card

eval_card = function(card, context)
	local ret, post = bunc_original_eval_card(card, context)

	if context
		and context.main_scoring
		and context.cardarea == G.play
		and card
		and card:is_suit("bunc_Halberds")
		and context.scoring_hand
		and #context.scoring_hand > 0
	then
		local target = pseudorandom_element(
			context.scoring_hand,
			pseudoseed("bunc_halberd")
		)

		target.ability.perma_mult =
			(target.ability.perma_mult or 0) + 1
...
function Game:update(dt)
	...
	if
		G.GAME
		and G.GAME.blind
		and G.GAME.blind.name == "bl_bunc_wind"
		and G.GAME.blind.ready
	...
	if
		...
		and G.GAME.round_resets.blind_choices.Boss == "bl_bunc_final_trident"
	then
		G.GAME.Trident = true
	else
...
function BUNCOMOD.content.set_debuff(card)
    ...
    if my_pos then
        if G.jokers.cards[my_pos - 1]
        and G.jokers.cards[my_pos - 1].edition
        and G.jokers.cards[my_pos - 1].edition.bunc_fluorescent then
            return 'prevent_debuff'
        end
	...
	if
		G.GAME.blind.name == "bl_bunc_prince"
		and G.GAME.blind.ready
		and not G.GAME.blind.disabled
	...
	if my_pos then
		if
			G.jokers.cards[my_pos - 1]
			and G.jokers.cards[my_pos - 1].config.center.key == "j_bunc_gameplan"
			and not G.jokers.cards[my_pos - 1].debuff
		then
			return true
		end
		if
...
function Game:start_run(args)
	...
	if #sledgehammers >= 1 then
		G.P_CENTERS.m_glass.config.extra = G.P_CENTERS.m_glass.config.extra
			/ G.P_CENTERS["j_bunc_sledgehammer"].config.extra.div_chance_denom
	end
end
...
bunc_define_joker({ -- Cassette
	...
	custom_vars = function(self, info_queue, card)
		...
		localize({
			type = "descriptions",
			set = "Joker",
			key = "j_bunc_cassette_" .. (card.ability.extra.side == "A" and "b" or "a"),
			nodes = side_nodes,
			vars = vars,
			scale = scale,
		})
		...
		local main_end = {
			{
				...
				nodes = {
					{ n = G.UIT.R, config = { align = "cm" }, nodes = flip_nodes[1] },
					{
						n = G.UIT.R,
								...
								nodes = {
									{
										...
										config = {
											object = DynaText({
												string = {
													G.localization.misc.dictionary["bunc_" .. (card.ability.extra.side == "A" and "b" or "a") .. "_side"],
												},
												colours = { G.C.WHITE },
		...
		if card.ability.extra.side == "A" then
			...
			return { key = self.key .. "_a", main_end = main_end, vars = vars }
		else
			if next(SMODS.find_mod("paperback")) then
			...
			return { key = self.key .. "_b", main_end = main_end, vars = vars }
		end
	end,
	rarity = "Uncommon",
	cost = 5,
	...
	calculate = function(self, card, context)
		...
		if context.individual and context.cardarea == G.play then
			...
			if
				...
				or other_card:is_suit("bunc_Fleurons")
			then
				if side == "A" then
					return {
	...
	calculate = function(self, card, context)
		if context.individual and context.cardarea == G.play then
			...
			if not SMODS.has_no_rank(other_card) then
				...
				elseif
					other_card:get_id() == 12
					or other_card:get_id() == 10
					or other_card:get_id() == 9
					or other_card:get_id() == 6
				then
		...
		if not context.blueprint then
			...
			if (context.after or context.first_hand_drawn) and G.GAME.current_round.hands_left <= 1 then -- For shaking the card when there's one hand left
				event({
					func = function()
						local eval = function()
							return G.GAME.current_round.hands_left == nil or G.GAME.current_round.hands_left ~= 0
						end
				...
				return {
					...
					func = function()
						...
						card.ability.extra.level_up_list[context.scoring_name] = (
							card.ability.extra.level_up_list[context.scoring_name] or 0
	...
	calculate = function(self, card, context)
		if context.individual and context.cardarea == G.play then
			for _, v in ipairs(card.ability.extra.card_list) do
				if
					(aij_ids_op(context.other_card, "==", v.rank))
					and (v.has_any_suit or context.other_card:is_suit(v.suit))
				then
					return {
						mult = card.ability.extra.mult,
					}
		...
		if context.after and context.scoring_hand and #context.scoring_hand == 2 and not context.blueprint then
			if context.poker_hands and next(context.poker_hands["Pair"]) then
				...
				event({
					...
					delay = 0.1,
					func = function()
						context.scoring_hand[1]:change_suit(context.scoring_hand[2].config.card.suit)
						return true
					end,
				})
				event({
...
bunc_define_joker({ -- Shepherd
	...
	calculate = function(self, card, context)
		...
		then
			...
			return {
				message = "+"
					.. tostring(card.ability.extra.chips)
					.. " "
					.. G.localization.misc.dictionary["bunc_chips"],
				colour = G.C.BLUE,
			}
		end
...
bunc_define_joker({ -- Joker Man & Jester Boy Trading Card No. 54 (JMJB)
	name = "JMJB",
	position = 15,
	vars = { { unlock = 50 } },
	locked_vars = function(self, info_queue, card)
	...
	calculate = function(self, card, context)
		if context.open_booster and context.card.ability.name then
			if
				context.open_booster and context.card.ability.name == "Standard Pack"
				or context.open_booster and context.card.ability.name == "Jumbo Standard Pack"
				or context.open_booster and context.card.ability.name == "Mega Standard Pack"
			then
				event({
					trigger = "after",
					delay = 0,
		...
		if context.joker_main then
			...
			if context.scoring_hand then
				for i = 1, #context.scoring_hand do
					if
						not (
							context.scoring_hand[i]:get_id() == 2
							or context.scoring_hand[i]:get_id() == 3
							or context.scoring_hand[i]:get_id() == 4
							or context.scoring_hand[i]:get_id() == 5
						) or SMODS.has_no_rank(context.scoring_hand[i])
					then
						condition = false
					end
				end
...
bunc_define_joker({ -- Fiendish
	...
	calculate = function(self, card, context)
		if context.bunc_pre_money_change and context.sign == 1 then
			if
				SMODS.pseudorandom_probability(card, "fiendish" .. G.SEED, 1, card.ability.extra.odds, "bunc_fiendish")
			then
...
bunc_define_joker({ -- Carnival
	...
	custom_vars = function(self, info_queue, card)
		local active = (G.GAME and G.GAME.round_resets and (G.GAME.round_resets.ante > card.ability.extra.ante))
			or false
		local main_end = {
			{
				n = G.UIT.C,
						...
						nodes = {
							{
								n = G.UIT.T,
								config = {
									text = " " .. localize(active and "k_active" or "bunc_inactive") .. " ",
									colour = G.C.UI.TEXT_LIGHT,
									scale = 0.32 * 0.9,
								},
							},
		...
		if context.end_of_round and G.GAME.blind.boss and not context.other_card and not context.blueprint then
			if G.GAME.round_resets.ante > card.ability.extra.ante then
				...
				local joker_to_destroy = #destructable_jokers > 0
						and pseudorandom_element(destructable_jokers, pseudoseed("carnival"))
					or nil
			...
			if card.ability.extra.scoring_card_set then
				for _, v in ipairs(G.playing_cards) do
					if card.ability.extra.scoring_card_set[v.unique_val] then
						...
						v.ability.temporary_extra_chips = (v.ability.temporary_extra_chips or 0)
	...
	calculate = function(self, card, context)
		if context.individual and context.cardarea == G.play then
			if
				...
				or SMODS.has_no_rank(context.other_card)
			then
				if
					SMODS.pseudorandom_probability(
						card,
						pseudorandom("zero_shapiro" .. G.SEED),
						1,
...
local bunc_slothful_sprite_change = function(card)
	if BUNCOMOD.funcs.exotic_in_pool() then
		if
			next(SMODS.find_mod("paperback"))
			and (SMODS.Suits.paperback_Crowns:in_pool() or SMODS.Suits.paperback_Stars:in_pool())
		then
...
bunc_define_joker({ -- Gameplan
	...
	calculate = function(self, card, context)
		if context.other_joker then
			...
			if context.other_joker == G.jokers.cards[my_pos - 1] then
				mult = card.ability.extra.mult
			end
			if context.other_joker == G.jokers.cards[my_pos + 1] then
				mult = card.ability.extra.mult
...
bunc_define_joker({ -- Conquest
	...
	remove = function(self, card)
		if G.jokers then
			for i = 1, #G.jokers.cards do
				if G.jokers.cards[i].ability.bunc_conquest_chosen == card.unique_val then
					G.jokers.cards[i].ability.bunc_conquest_chosen = nil
				end
				G.GAME.blind:debuff_card(G.jokers.cards[i])
			end
		end
					...
					func = function()
						if not card.debuffed then
							card_eval_status_text(card, "extra", nil, percent, nil, {
								message = G.localization.misc.dictionary["bunc_debuffed"],
								colour = G.C.RED,
								instant = true,
							})
						end
...
bunc_define_joker({ -- Dwarven
	...
	add = function(self, card)
		...
		G.P_CENTERS.m_stone.config.h_x_mult = (G.P_CENTERS.m_stone.config.h_x_mult or 0)
			+ G.P_CENTERS.m_steel.config.h_x_mult
		G.P_CENTERS.m_stone.config.h_dollars = (G.P_CENTERS.m_stone.config.h_dollars or 0)
			+ G.P_CENTERS.m_gold.config.h_dollars
	end,
	remove = function(self, card)
		for _, deck_card in pairs(G.playing_cards) do
			if deck_card.config.center == G.P_CENTERS.m_stone then
		...
		G.P_CENTERS.m_stone.config.h_dollars = G.P_CENTERS.m_stone.config.h_dollars
			- G.P_CENTERS.m_gold.config.h_dollars
	end,
})
...
bunc_define_joker({ -- Metallurgist
	...
	add = function(self, card)
		for _, deck_card in pairs(G.playing_cards) do
			if deck_card.config.center == G.P_CENTERS.m_gold then
				deck_card.ability.h_mult = (
					deck_card.ability.h_mult and (deck_card.ability.h_mult + card.ability.extra.mult) or 0
				)
			end
		end
	...
	remove = function(self, card)
		...
		G.P_CENTERS.m_gold.config.h_mult = (
			G.P_CENTERS.m_gold.config.h_mult and (G.P_CENTERS.m_gold.config.h_mult - card.ability.extra.mult) or 0
		)
	end,
})
...
bunc_define_joker({ -- Head in the Clouds
	...
	custom_in_pool = function()
		...
		if G.GAME and G.GAME.hands then
			if G.GAME.hands["High Card"].level > to_big(1) then
				condition = true
			end
		end
		return condition
	...
	calculate = function(self, card, context)
		if context.level_up_hand and context.level_up_hand ~= self.name then
			if
				SMODS.pseudorandom_probability(
					card,
					"head_in_the_clouds" .. G.SEED,
					1,
					card.ability.extra.odds,
					"bunc_head_in_the_clouds"
				)
			then
				event({
					func = function()
...
bunc_define_joker({ -- Trigger Finger
	...
	custom_vars = function(self, info_queue, card)
		local vars = {
			...
			SMODS.get_probability_vars(card, 1, card.ability.extra.odds, "bunc_trigger_finger"),
		}
		return { vars = vars }
	end,
	rarity = "Rare",
	...
	calculate = function(self, card, context)
		if
			context.bunc_trigger_finger_highlight_card
...
bunc_define_joker({ -- Vandalism
	name = "Vandalism",
	position = 42,
	soul = coordinate_from_atlas_index(42),
	...
	set_sprites = function(self, card, front)
		if card.config.center.discovered or card.bypass_discovery_center then
			card.children.center = Sprite(
				...
				card.T.w,
				card.T.h,
				G.ASSET_ATLAS[(G.GAME.viewed_back or G.GAME.selected_back) and ((
					G.GAME.viewed_back or G.GAME.selected_back
				)[G.SETTINGS.colourblind_option and "hc_atlas" or "lc_atlas"] or (
					G.GAME.viewed_back or G.GAME.selected_back
				).atlas) or "centers"],
			...
			card.children.back = Sprite(
				...
				card.T.w,
				card.T.h,
				G.ASSET_ATLAS["bunc_bunco_jokers_winking"],
				coordinate_from_atlas_index(1)
			)
	...
	drawsteps = {
		{
			...
			func = function(card, layer)
				if
					card.config.center.key == "j_bunc_vandalism"
...
bunc_define_joker({ -- Doodle
	...
	check_for_unlock = function(self, args)
		if
			args.type == "win_custom"
			and (G.PROFILES[G.SETTINGS.profile].career_stats.c_wins + 1) >= self.config.extra.unlock
		then
			unlock_card(self)
		end
	end,
...
bunc_define_joker({ -- Critic
	...
	calculate = function(self, card, context)
		if context.joker_main then
			...
			if math.floor(hand_chips * mult) < (temp_chips / card.ability.extra.fraction) then
				return {
					xmult = card.ability.extra.xmult,
				}
			end
...
bunc_define_joker({ -- Bounty Hunter
	...
	check_for_unlock = function(self, args)
		if args.type == "money" then
			if to_big(G.GAME.dollars) < to_big(self.config.extra.unlock) then
				unlock_card(self)
			end
		end
	end,
...
bunc_define_joker({ -- Mousetrap
	...
	custom_vars = function(self, info_queue, card)
		local vars =
			{ card.ability.extra.chips, SMODS.get_probability_vars(card, 1, card.ability.extra.odds, "bunc_mousetrap") }
		return { vars = vars }
	end,
	rarity = "Uncommon",
	cost = 6,
	...
	calculate = function(self, card, context)
		if
			(context.end_of_round or context.hand_drawn or context.after)
			and card.children.center.sprite_pos.y == 15
		then
			G.E_MANAGER:add_event(Event({
				trigger = "immediate",
				func = function()
...
bunc_define_joker({ -- The Joker
	name = "The Joker",
	custom_atlas = "bunco_jokers_the_joker",
	position = 1,
	...
	calculate = function(self, card, context)
		if (context.buying_card or context.getting_booster_card) and context.pre_cardarea and not context.blueprint then
			local function acquire(acquired_card)
				...
				if acquired_card.children.price then
					acquired_card.children.price:remove()
				end
				acquired_card.children.price = nil
				if acquired_card.children.buy_button then
					acquired_card.children.buy_button:remove()
				end
			...
			if card_pos then
				...
				if left_card and left_card.area ~= G.consumeables then
					if
						G.FUNCS.check_for_buy_space(left_card)
						and (
							left_card.ability.consumeable
							or left_card.ability.set == "Enhanced"
							or left_card.ability.set == "Default"
							or left_card.ability.set == "Joker"
						)
					then
						acquire(left_card)
						big_juice(card)
				...
				if right_card and right_card.area ~= G.consumeables then
					if
						G.FUNCS.check_for_buy_space(right_card)
						and (
							right_card.ability.consumeable
							or right_card.ability.set == "Enhanced"
							or right_card.ability.set == "Default"
							or right_card.ability.set == "Joker"
						)
					then
						acquire(right_card)
						big_juice(card)
...
bunc_define_joker({ -- Glue Gun
	...
	custom_vars = function(self, info_queue, card)
		...
		local active = (
			...
			and (#G.hand.highlighted <= card.ability.extra.amount)
		) or false
		local main_end = {
			{
				n = G.UIT.C,
...
bunc_define_joker({ -- Taped
	name = "Taped",
	custom_atlas = "bunco_jokers_taped",
	position = 1,
	custom_vars = function(self, info_queue, card)
		info_queue[#info_queue + 1] = { set = "Other", key = "bunc_linked_group" }
	end,
...
bunc_define_joker({ -- Rubber Band Ball
	...
	update = function(self, card)
		card.ability.extra.xmult = (G.GAME.last_card_group and G.GAME.last_card_group + 1 or 1)
			* card.ability.extra.bonus
	end,
	custom_in_pool = function()
...
bunc_define_joker({ -- Headache
	name = "Headache",
	custom_atlas = "bunco_jokers_headache",
	position = 1,
	vars = { { amount = 4 }, { destroyed = 0 } },
	custom_vars = function(self, info_queue, card)
		info_queue[#info_queue + 1] = { set = "Tag", key = "tag_bunc_arcade" }
		info_queue[#info_queue + 1] = {
			key = "p_bunc_virtual_mega",
			set = "Other",
			vars = { G.P_CENTERS.p_bunc_virtual_mega.config.choose, G.P_CENTERS.p_bunc_virtual_mega.config.extra },
		}
	...
	calculate = function(self, card, context)
		if context.remove_playing_cards then
			...
			while card.ability.extra.destroyed >= card.ability.extra.amount do
				...
				forced_message(
					"+1 " .. localize({ type = "name_text", key = "tag_bunc_arcade", set = "Tag" }),
					card,
					G.C.BUNCO_VIRTUAL
				)
	...
	set_sprites = function(self, card, front)
		if card.config.center.discovered or card.bypass_discovery_center then
			card.children.floating_sprite = Sprite(
				...
				card.T.w,
				card.T.h,
				G.ASSET_ATLAS["bunc_bunco_jokers_border"],
				coordinate_from_atlas_index(1)
			)
	...
	drawsteps = {
		{
			...
			func = function(card, layer)
				if
					card.config.center.key == "j_bunc_headache"
					and (card.config.center.discovered or card.bypass_discovery_center)
				then
					local realw, realh = love.window.getMode()

					card.children.center:draw_shader("bunc_headache", nil, {
						[1] = ((card.tilt_var.mx - realw / 2) * 0.3)
							+ ((card.VT.x + G.ROOM.T.x + (card.VT.w / 2)) * G.TILESIZE * G.TILESCALE - realw / 2)
								* -0.2,
						[2] = ((card.tilt_var.my - realh / 2) * 0.8)
							+ ((card.VT.y + G.ROOM.T.y + (card.VT.h / 2)) * G.TILESIZE * G.TILESCALE - realh / 2)
								* -0.4,
						[3] = G.TIMERS.REAL,
					})
				end
			...
			if SMODS.has_no_rank(other_card) then
				...
				event({
					...
					func = function()
						play_sound(
							instrument,
							calculate_pitch(other_card:get_id()),
							2.0 * G.SETTINGS.SOUND.bunc_stylophone_volume / 100
						)
						return true
					end,
				})
	...
	calculate = function(self, card, context)
		if context.rescore_cards then
			...
			if
				...
				and SMODS.pseudorandom_probability(
					card,
					"kite_experiment" .. G.SEED,
					1,
					card.ability.extra.odds,
					"bunc_kite_experiment"
				)
			then
				context.rescore_cards[1].config.copper_rescored_times = context.rescore_cards[1].config.copper_rescored_times
					- 1
...
bunc_define_joker({ -- Hardtack
	...
	custom_in_pool = function()
		...
		if G.playing_cards then
			for k, v in pairs(G.playing_cards) do
				if v.config.center == G.P_CENTERS.m_bunc_cracker then
					condition = true
					break
				end
			end
...
bunc_define_joker({ -- Pica
	...
	custom_in_pool = function()
		...
		if G.playing_cards then
			for k, v in pairs(G.playing_cards) do
				if SMODS.has_enhancement(v, "m_bunc_cracker") then
					condition = true
					break
				end
			end
...
bunc_define_joker({ -- Dynasty
	...
	custom_config = { Xmult = 3, type = "bunc_Spectrum" },
	rarity = "Rare",
	cost = 8,
	blueprint = true,
	eternal = true,
...
bunc_define_joker({ -- Magic Wand
	...
	calculate = function(self, card, context)
		if
			context.before
			and context.poker_hands ~= nil
			and next(context.poker_hands["bunc_Spectrum"])
			and not context.blueprint
		then
			card.ability.extra.mult = card.ability.extra.mult + card.ability.extra.bonus
		elseif
			context.after
			and context.poker_hands ~= nil
			and not next(context.poker_hands["bunc_Spectrum"])
			and not context.blueprint
		then
			if card.ability.extra.mult ~= 0 then
				card.ability.extra.mult = 0
		...
		then
			...
			for i = 1, #context.scoring_hand do
				event({
					...
					delay = 0.1,
					func = function()
						context.scoring_hand[i]:change_suit("bunc_Fleurons")
						return true
					end,
				})
			end
...
BUNCOMOD.funcs.myopia_check = function(card, suit)
	...
	if
		card.base.suit == "Spades" and ((card.base.suit == "Spades") == (suit == "Spades" or suit == "bunc_Halberds"))
	then
		return true
	end
...
BUNCOMOD.funcs.astigmatism_check = function(card, suit)
	...
	if
		...
		and (card.base.suit == "Diamonds") == (suit == "Diamonds" or suit == "bunc_Fleurons")
	then
		return true
	end
	return false
...
bunc_define_joker({ -- ROYGBIV
	...
	calculate = function(self, card, context)
		...
		then
			if
				SMODS.pseudorandom_probability(card, "roygbiv" .. G.SEED, 1, card.ability.extra.odds, "bunc_roygbiv")
			then
				if context.scoring_hand then
					local cards = {}
...
bunc_define_joker({ -- Rigoletto
	...
	update = function(self, card)
		if G.playing_cards ~= nil then
			...
			for k, v in pairs(G.playing_cards) do
				if BUNCOMOD.funcs.is_exotic_suit(v) then
	card.ability.extra.tally = card.ability.extra.tally + 1
      end
			end
...
SMODS.Consumable({ -- Adjustment
	...
	set_card_type_badge = function(self, card, badges)
		badges[1] = create_badge(
			G.localization.misc.dictionary.bunc_thoth_tarot,
			get_type_colour(self or card.config, card),
			nil,
			1.2
		)
	...
	loc_vars = function(self, info_queue)
		...
		return {
			vars = {
				self.config.bonus,
				self.config.limit,
				(reward <= self.config.limit) and reward or self.config.limit,
			},
		}
	end,
...
SMODS.Consumable({ -- The Sky
	set = "Tarot",
	atlas = "bunco_tarots_exotic",
	key = "sky",
	set_card_type_badge = function(self, card, badges)
		badges[1] = create_badge(
			G.localization.misc.dictionary.bunc_mysterious_tarot,
			get_type_colour(self or card.config, card),
			nil,
			1.2
		)
...
function create_polymino_card_set(example)
	local cardarea =
		CardArea(0, 0, 2.85 * G.CARD_W, 0.75 * G.CARD_H, { card_limit = 4, type = "title", highlight_limit = 0 })
	...
	func = function(card, layer)
		if card.ability.group and card.area ~= G.deck then
			if not card.children.link then
				...
				card.children.link = Sprite(
					card.T.x,
					card.T.y,
					card.T.w,
					card.T.h,
					G.ASSET_ATLAS["bunc_bunco_link"],
					{ x = variant - 1, y = 0 }
				)
			...
			func = function()
				...
				while n < #G.deck.cards do
					...
					if other_card == card or other_card.area == G.hand or not other_card then
						...
						if
							other_card.ability.group
							and other_card.ability.group.id == card.ability.group.id
							and other_card.area == G.deck
						then
							event({
								trigger = "immediate",
								func = function()
...
function CardArea:add_to_highlighted(card, silent)
	if G.STATE ~= G.STATES.DRAW_TO_HAND and not G.DRAWING_CARDS then
		...
		self.config.highlighted_limit = G.GAME.THE_8_BYPASS and self.config.type == "hand" and math.huge
			or self.config.highlighted_limit
		...
		if card.ability.group and self then
			...
			for i = 1, #self.cards do
				if self.cards[i].ability.group and self.cards[i].ability.group.id == card.ability.group.id then
					if
						self.config.type == "hand"
						and not self.cards[i].highlighted
						and not (
							card ~= self.cards[i]
							and self.cards[i].edition
							and self.cards[i].edition.bunc_fluorescent
						)
					then
			...
			for i = 1, #group do
				if i ~= #group then
					...
					if
						G.GAME.blind
						and G.GAME.blind.name == "bl_bunc_gate"
						and not G.GAME.blind.disabled
						and self == G.hand
					then
	...
	if card.ability.group and self and not force then
		...
		for i = 1, #self.cards do
			if
				...
				and self.cards[i].ability.group.id == card.ability.group.id
			then
				table.insert(group, self.cards[i])
			end
		end
...
SMODS.Consumable({ -- The I
	...
	can_use = function(self, card)
		if
			G.hand
			and (#G.hand.highlighted == 4 or (G.GAME.used_vouchers["v_bunc_polybius"] and #G.hand.highlighted == 5))
		then
...
SMODS.Consumable({ -- The /
	...
	set_card_type_badge = function(self, card, badges)
		badges[1] = create_badge(
			G.localization.misc.dictionary.bunc_mysterious_polymino,
			get_type_colour(self or card.config, card),
			nil,
			1.2
		)
...
SMODS.Suit({ -- Halberds
  ...
  loc_txt = {
	...
	collection = "Halberds",
	description = {
		"Give a random scored card",
		"+1 permanent Mult when scored"
	}
},
...
SMODS.Atlas({
	key = "bunco_resprites_enhanced_contrast_ui",
	path = "Resprites/EnhancedUIContrast.png",
	px = 18,
	py = 18,
})
SMODS.Atlas({
	key = "bunco_paperback_enhanced_contrast",
	path = "Resprites/EnhancedContrastPaperback.png",
	px = 71,
	py = 95,
})
...
for i, suit in ipairs({ "Hearts", "Diamonds", "Clubs", "Spades" }) do
	SMODS.DeckSkin.add_palette(SMODS.DeckSkins["default_" .. suit], {
		...
		pos_style = {
			...
			["paperback_Apostle"] = {
				pos = { x = 0, y = ({ 0, 2, 1, 3 })[i] },
				atlas = "bunc_bunco_paperback_enhanced_contrast",
			},
		},
	...
	for _, collab in pairs(collabs[suit]) do
		SMODS.Atlas({
			key = "collab_" .. collab .. "_alt_hc",
			path = "Resprites/collabs/collab_" .. collab .. "_EnhancedContrast.png",
			px = 71,
			py = 95,
		})
...
SMODS.PokerHand({ -- Straight Spectrum (Referenced from SixSuits)
	...
	evaluate = function(parts)
		if not next(parts.bunc_spectrum) or not next(parts._straight) then
			return {}
		end
...
SMODS.PokerHand({ -- Spectrum House (Referenced from SixSuits)
	...
	evaluate = function(parts)
		if #parts._3 < 1 or #parts._2 < 2 or not next(parts.bunc_spectrum) then
			return {}
		end
...
SMODS.PokerHand({ -- Spectrum Five (Referenced from SixSuits)
	...
	evaluate = function(parts)
		if not next(parts._5) or not next(parts.bunc_spectrum) then
			return {}
		end
...
SMODS.Atlas({
	key = "bunco_blinds_finisher",
	path = "Blinds/BlindsFinisher.png",
	px = 34,
	py = 34,
	frames = 21,
	atlas_table = "ANIMATION_ATLAS",
...
BUNCOMOD.funcs.bunc_refresh_boss_blind = function()
	if G.GAME.blind.boss or not G.blind_select_opts then
		return
	end
...
SMODS.Blind({ -- The Tine
	...
	recalc_debuff = function(self, card, from_blind)
		if not G.GAME.blind.disabled and card.area ~= G.jokers and not SMODS.has_no_rank(card) then
...
SMODS.Blind({ -- The Mask
	...
	modify_hand = function(self, cards, poker_hands, text, mult, hand_chips)
		...
		if not G.GAME.blind.disabled then
			if G.GAME.last_hand_played == G.GAME.current_round.most_played_poker_hand then
				...
				return G.GAME.hands[G.GAME.current_round.least_played_poker_hand].s_mult,
					G.GAME.hands[G.GAME.current_round.least_played_poker_hand].s_chips,
					true
			end
		end
	...
	in_pool = function(self)
		if
			(G.GAME.round_resets.ante >= self.boss.min)
			and G.GAME.current_round.most_played_poker_hand ~= G.GAME.current_round.most_played_poker_hand
		then
			return true
		else
			return false
...
SMODS.Blind({ -- The Knoll
	...
	stay_flipped = function(self, area, card)
		if
			...
			and #(G.GAME.Knoll or {}) < G.hand.config.card_limit
		then
	...
	set_blind = function(self, reset, silent)
		...
		if not reset and not G.GAME.blind.disabled and to_big(G.GAME.dollars) >= to_big(10) then
			local final_chips = (G.GAME.blind.chips / G.GAME.blind.mult)
				* (math.floor(to_number(G.GAME.dollars) / 10) + G.GAME.blind.mult)
...
SMODS.Blind({ -- The Blade
	...
	vars = {},
	loc_vars = function(self)
		local overscore = get_blind_amount(G.GAME.round_resets.ante)
			* G.P_BLINDS.bl_bunc_blade.mult
...
SMODS.Blind({ -- Chartreuse Crown
	...
	recalc_debuff = function(self, card, from_blind)
		if not G.GAME.blind.disabled and card.area ~= G.jokers then
			if
				...
				or card.base.suit == "Hearts"
				or card.base.suit == "Clubs"
				or card.base.suit == "Diamonds"
			then
	...
	in_pool = function()
		...
		if G.playing_cards then
			for k, v in pairs(G.playing_cards) do
				if BUNCOMOD.funcs.is_base_exotic_suit(v) then
...
SMODS.Back({ -- Fairy
	...
	loc_vars = function(self)
		return {
			vars = { self.config.amount, localize({ type = "name_text", set = "Other", key = "bunc_exotic_cards" }) },
		}
	end,
    ...
    if context.round_eval and G.GAME.last_blind and G.GAME.last_blind.boss then
        event({
            func = function()
                ...
                local suits = BUNCOMOD.funcs.get_exotic_suits()
                local cards = {}

                for i = 1, self.config.amount do
                    local rank = pseudorandom_element(
                        numbers,
                        pseudoseed("fairy_rank_" .. G.SEED .. "_" .. i)
                    )

                    local suit = pseudorandom_element(
                        suits,
                        pseudoseed("fairy_suit_" .. G.SEED .. "_" .. i)
                    )

                    local suit_data = SMODS.Suits[suit]

                    if suit_data then
                        local card_key = suit_data.card_key
                        local front = G.P_CARDS[card_key .. "_" .. rank]
...
SMODS.Tag({ -- Triple
	...
	apply = function(self, tag, context)
		if
			...
			and context.tag.key ~= "tag_bunc_triple"
		then
			G.CONTROLLER.locks[tag.ID] = true
			tag:yep("+", G.C.BUNCO_VIRTUAL, function()
	...
	in_pool = function()
		return G.GAME.used_vouchers["v_bunc_pin_collector"]
	end,
})
			...
			event({
				...
				func = function()
					...
					then
						...
						for _, v in ipairs(G.pack_cards.cards) do
	if
		not BUNCOMOD.funcs.is_exotic_suit(v)
		or SMODS.has_any_suit(v)
	then
		local suits = BUNCOMOD.funcs.get_exotic_suits()
...
SMODS.Tag({ -- Perishable
	...
	loc_vars = function(self, info_queue)
		info_queue[#info_queue + 1] = {
			key = "perishable",
			set = "Other",
			vars = { G.GAME.perishable_rounds or 1, G.GAME.perishable_rounds or G.GAME.perishable_rounds },
		}
		return {}
	end,
...
function Card:create_blind_card()
	if
		pseudorandom("the_joker" .. G.SEED) < 0.04
		and G.P_CENTERS["j_bunc_the_joker"].unlocked
		and not (G.GAME.used_jokers["j_bunc_the_joker"] and not next(find_joker("Showman")))
	then
	...
	self.children.center = Sprite(
		...
		self.T.w,
		self.T.w,
		G.ANIMATION_ATLAS[blind.atlas] or G.ANIMATION_ATLAS["blind_chips"],
		{ x = blind.pos.x or 0, y = blind.pos.y or 0 }
	)
...
for i = 1, 4 do -- Blind
	SMODS.Booster({
															...
															nodes = {
																{
																	...
																	config = {
																		object = DynaText({
																			string = localize(
																				self.group_key or "k_bunc_blind_pack"
																			),
																			colours = { G.C.WHITE },
																			shadow = true,
																			rotate = true,
											...
											(
												G.GAME.used_vouchers["v_bunc_shell_game"]
													...
													and UIBox_button({
														...
														r = 0.15,
														one_press = true,
														button = "reroll_booster_pack",
														func = "reroll_booster_pack_button",
													})
												or nil
											),
											G.GAME.used_vouchers["v_bunc_shell_game"]
												and { n = G.UIT.R, config = { minh = 0.07 }, nodes = {} },
											{
												n = G.UIT.R,
												config = {
													align = G.GAME.used_vouchers["v_bunc_shell_game"] and "cm" or "tm",
													padding = G.GAME.used_vouchers["v_bunc_shell_game"] and 0.07 or 0.2,
													minh = G.GAME.used_vouchers["v_bunc_shell_game"] and 0.7 or 1.2,
													minw = 1.8,
													r = 0.15,
													colour = G.C.GREY,
													one_press = true,
													...
													hover = true,
													shadow = true,
													func = "can_skip_booster",
												},
												nodes = {
													{
														n = G.UIT.T,
														config = {
															text = localize("b_skip"),
															scale = 0.5,
															colour = G.C.WHITE,
															shadow = true,
															focus_args = { button = "y", orientation = "bm" },
															func = "set_button_pip",
														},
													},
												},
		...
		in_pool = function()
			return G.GAME.used_vouchers["v_bunc_disguise"]
		end,
	})
end
...
for i = 1, 4 do -- Virtual
	SMODS.Booster({
		...
		loc_vars = function(self, info_queue, card)
			return {
				key = (self.key == "p_bunc_virtual_1" or self.key == "p_bunc_virtual_2") and "p_bunc_virtual_normal"
					or self.key,
				vars = { self.config.choose, self.config.extra },
			}
		end,
															...
															nodes = {
																{
																	...
																	config = {
																		object = DynaText({
																			string = localize(
																				self.group_key or "k_bunc_virtual_pack"
																			),
																			colours = { G.C.WHITE },
																			shadow = true,
																			rotate = true,
...
function calculate_cracker_cards(context)
	if #SMODS.find_card("j_bunc_hardtack", false) <= 0 then
		...
		for _, cracker_card in ipairs(context.full_hand) do
			if
				cracker_card.marked_cracker
				and SMODS.has_enhancement(cracker_card, "m_bunc_cracker")
				and not cracker_card.debuff
			then
...
SMODS.Enhancement({ -- Copper
	...
	calculate = function(self, card, context, effect)
		if
			...
			and context.scoring_hand
			and (
				not card.config.copper_rescored_times
				or (card.config.copper_rescored_times < card.config.center.rescore_amount)
			)
		then
			local card_position
			...
			if
				context.scoring_hand[card_position + 1]
				and context.scoring_hand[card_position + 1].config.center == card.config.center
				and not context.scoring_hand[card_position + 1].debuff
			then
					...
					return {
						func = function()
							while (card.config.copper_rescored_times or 0) < card.config.center.rescore_amount do
								...
								while true do
									if
										context.scoring_hand[card_position - i]
										and context.scoring_hand[card_position - i].config.center == card.config.center
										and not context.scoring_hand[card_position - i].debuff
									then
								...
								for streak_index = #streak_cards, 1, -1 do
									...
									for _, play_card in ipairs(G.play.cards) do
										if play_card == streak_card then
											streak_card.config.copper_rescored_times = (
												streak_card.config.copper_rescored_times or 0
											) + 1
...
SMODS.Sticker({ -- Scattering
	...
	should_apply = function(self, card, center, area, bypass_roll)
		...
		local other_sticker_check = not (
			(card and card.ability)
			and (card.ability.eternal or card.ability.bunc_hindered or card.ability.bunc_reactive)
		)
...
SMODS.Sticker({ -- Hindered
	...
	should_apply = function(self, card, center, area, bypass_roll)
		...
		local other_sticker_check = not (
			(card and card.ability)
			and (card.ability.eternal or card.ability.bunc_scattering or card.ability.bunc_reactive)
		)
		local shop_check = (area == G.shop_jokers) or (area == G.pack_cards)
		return default_check and other_sticker_check and shop_check
	end,
...
SMODS.Sticker({ -- Reactive
	...
	should_apply = function(self, card, center, area, bypass_roll)
		local default_check = SMODS.Sticker.should_apply(self, card, center, area, bypass_roll)
		local other_sticker_check = not (
			(card and card.ability) and (card.ability.bunc_hindered or card.ability.bunc_scattering)
		)
		local shop_check = (area == G.shop_jokers) or (area == G.pack_cards)
		return default_check and other_sticker_check and shop_check
	end,
...
for i, v in ipairs(consumable_editions) do
	SMODS.Edition({
		...
		loc_vars = function(self, info_queue, card)
			info_queue[#info_queue + 1] =
				{ key = v.tag, set = "Tag", specific_vars = G.P_TAGS[v.tag].loc_vars(self, info_queue).vars }
